...
export type FirecrawlScrapePayload =
  | FirecrawlScrapeSuccessPayload
  | FirecrawlScrapeFailurePayload
...
const requestFirecrawl = async <T>({ apiKey, path, init }: FirecrawlRequestConfig): Promise<{
  data?: T
  status: number
}> => {
...
export const performScrape = async (
  ...
  const primaryFormatSpec = R.find(formats, (format) => {
